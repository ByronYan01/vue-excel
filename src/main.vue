...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    function renderExcel(buffer) {
      fileData = buffer;
      readExcelData(buffer, props.options.xls)
        .then((workbook) => {
          if (!workbook._worksheets || workbook._worksheets.length === 0) {
            throw new Error("未获取到数据，可能文件格式不正确或文件已损坏");
          }
          if (
            props.options.beforeTransformData &&
            typeof props.options.beforeTransformData === "function"
          ) {
            workbook = props.options.beforeTransformData(workbook);
          }
          let { workbookData, medias, workbookSource } =
            transferExcelToSpreadSheet(workbook, {
              ...defaultOptions,
              ...props.options,
            });
          if (
            props.options.transformData &&
            typeof props.options.transformData === "function"
          ) {
          ...
          renderImage(
            ctx,
            mediasSource,
            workbookDataSource._worksheets[sheetIndex],
            offset,
            props.options
          );
          emit("rendered");
    ...
    onMounted(() => {
      nextTick(() => {
        ...
        xs = new Spreadsheet(rootRef.value, {
          ...
          view: {
            height: () =>
              (wrapperRef.value && wrapperRef.value.clientHeight) || 300,
            width: () =>
              (wrapperRef.value && wrapperRef.value.clientWidth) || 1200,
          },
          row: {
            height: 24,
            len: 100,
    ...
    function save(fileName) {
      downloadFile(
        fileName || `xz-vue-excel-${new Date().getTime()}.xlsx`,
        fileData
      );
    }
    return {
...
<template>
  <div class="xz-vue-excel" ref="wrapperRef">
    <div class="xz-vue-excel-main" ref="rootRef"></div>
  </div>
</template>
